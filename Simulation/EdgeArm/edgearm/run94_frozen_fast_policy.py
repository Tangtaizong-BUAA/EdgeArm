"""Frozen Run93 inference with only the independent cohort guard changed.

Run94 may supply an adapted action head; all sensor and physical semantics
remain exactly those used by its completed development evaluation.
"""
...
def episode(job):
    seed, checkpoint, vision_path, control_path, keypoint_path, output, mode, groups = job
    if mode != "memory_fast_keypoints":
        raise ValueError("frozen high-frequency keypoint policy required")
